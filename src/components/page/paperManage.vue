<template>
  <div class="paper-box">
    <el-upload class="upload-demo" drag action="https://morty.xiaomy.net/api/bigData/bTopic/insertTopic" multiple :before-upload="beforeAvatarUpload">
...
export default {
    methods: {
        beforeAvatarUpload(file) {
            ...
            if (file.name.split('.')[file.name.split('.').length - 1] != 'docx' && file.name.split('.')[file.name.split('.').length - 1] != 'doc') {
        ...
        async uploadPaper(file) {
            ...
            const res = await Axios.post('/api/bigData/bTopic/insertTopic', formdata, {
